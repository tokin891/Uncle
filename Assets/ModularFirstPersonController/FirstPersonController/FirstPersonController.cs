...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + mX * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * mY;
            }
            else
            {
                // Inverted Y
        ...
        //            if (t.position.x < 0) // Half screen
        //            {
        //                mX = t.deltaPosition.x;
        //                mY = t.deltaPosition.y;
        //            }
        //            break;
    ...
    public void CameraAxis(InputAction.CallbackContext context)
    {
        Vector2 axis = context.ReadValue<Vector2>() * .3f;
        mX = axis.x;
        mY = axis.y;
    }
}
